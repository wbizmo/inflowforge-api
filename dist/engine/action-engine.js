...
async function executeEmailAction(action, input) {
...
async function executeTelegramAction(action, input) {
...
async function executeHttpRequestAction(action, input) {
    const method = String(action.method || "POST").toUpperCase();
    const url = resolveTemplate(action.url, input);
    const headers = resolveObjectTemplates(action.headers ?? {}, input);
...
async function executeLogAction(action, input) {
    const message = resolveTemplate(action.message, input);
    return {
        type: "log",
        status: "SUCCESS",
...
export async function executeActions({ actions, input }) {
    const results = [];
